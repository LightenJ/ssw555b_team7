...
def us41_accept_partial_dates(individuals, families ):
        ...
        for ind in individuals:
            if "Birthday" in ind  and ind["Birthday"] == None:
                my_error = "Error!!! ..." + str(ind["ID"]) + " has partial Birthdate."

        for family in families:
            if "BIRTH DATE" in family and family["BIRTH DATE"] == None:
                my_error = "Error!!! ..." + str(family["ID"]) + " has partial Birthdate."


        for family in families:
            if "DEATH DATE" in family and family["DEATH DATE"] == None:
                my_error = "Error!!! ..." + str(family["FAMID"]) + " Has partial Death Date."
...
def us42_reject_illegitimate_dates(individuals):
    ...
    for k, v in individuals.items():
        ...
        if v['BIRTH DATE'] != "NA":
            ...
            ind_bday.append([k[1:-1], v['BIRTH DATE'], v['BIRTH DATE'].split(' ')[0], v['BIRTH DATE'].split(' ')[1],
                            v['BIRTH DATE'].split(' ')[2]])

            # limit => year
            if int(v['BIRTH DATE'].split(' ')[0]) > 0 and int(v['BIRTH DATE'].split(' ')[0]) <= 2021:
                fib = fib + 1

            # limit => month
            # if int(v['Birthday'].split(' ')[1]) > 0 and int(v['Birthday'].split(' ')[1]) <= 12:
                month = ['JAN', 'FEB', 'MAR', "APR", "MAY", "JUN", "JUL", "AUG", "SEP", "OCT", "NOV", "DEC"]
                m = v['BIRTH DATE'].split(' ')[1]

                for key, value in enumerate(month, start=1):
                    if (m == value):
                        m = key

                if (m > 0 and m <= 12):  # month
                    fib = fib + 1


            # limitn => day
            if int(v['BIRTH DATE'].split(' ')[2]) > 0 and int(v['BIRTH DATE'].split(' ')[2]) <= 31:
                fib = fib + 1

            if fib != 3:
                my_error = "Error: US42: Individual " + k[1:-1] +"'s birthday date is in illegitimate date format"

        fid = 0
        if v['DEATH DATE'] != "NA":
            # id,birthday,year,month,date
            ind_dday.append([k[1:-1], v['DEATH DATE'], v['DEATH DATE'].split(' ')
            [0], v['DEATH DATE'].split(' ')[1], v['DEATH DATE'].split(' ')[2]])
            if int(v['DEATH DATE'].split(' ')[0]) > 0 and int(v['DEATH DATE'].split(' ')[0]) <= 2020:  # year
                    fid = fid + 1

            month = ['JAN', 'FEB', 'MAR', "APR", "MAY", "JUN", "JUL", "AUG", "SEP", "OCT", "NOV", "DEC"]
            m = v['DEATH DATE'].split(' ')[1]                                                    # month
            ...
            if int(v['DEATH DATE'].split(' ')[2]) > 0 and int(v['DEATH DATE'].split(' ')[2]) <= 31:  # date
                    fid = fid + 1

            if fid != 3:
                    print("Error: US42: Individual " + k[1:-1] +"'s death date is in illegitimate date format")
    return my_error
